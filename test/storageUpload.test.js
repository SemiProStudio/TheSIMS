...
const { storageService, STORED_IMAGE_LIMIT_BYTES, UPLOAD_TIMEOUT_MS } =
...
function makeBucket({ fullError = null, thumbError = null } = {}) {
  ...
  const bucket = {
    ...
    getPublicUrl: vi.fn((path) => ({
      data: {
        publicUrl: `https://proj.supabase.co/storage/v1/object/public/equipment-images/${path}`,
      },
    })),
  };
...
describe('storageService.uploadRenditions — stalled network', () => {
  it('rejects with a timeout instead of hanging when the upload never settles', async () => {
    ...
    try {
      ...
      const pending = storageService.uploadRenditions(
        { full: blob(1000), thumb: blob(100) },
        'ITEM1',
      );
