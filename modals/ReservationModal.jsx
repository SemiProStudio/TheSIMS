          ...
          {searchResults.length === 0 ? (
            ...
            searchResults.map((item) => (
              <div
                ...
                onMouseEnter={(e) =>
                  (e.currentTarget.style.background = withOpacity(colors.primary, 15))
                }
  ...
  return (
    <div
      style={{
        ...
        gap: spacing[3],
        padding: spacing[3],
        background: hasConflict ? withOpacity(colors.warning, 10) : withOpacity(colors.primary, 10),
...
export const ReservationModal = memo(function ReservationModal({
  ...
  // Validation
  const dateValid =
    reservationForm.start && reservationForm.end && reservationForm.end >= reservationForm.start;
  const hasItems = selectedItems.length > 0;
  const valid =
    hasItems && reservationForm.project?.trim() && dateValid && reservationForm.user?.trim();
  const dateError =
    reservationForm.start && reservationForm.end && reservationForm.end < reservationForm.start;
      ...
      <div style={{ padding: spacing[4], maxHeight: '75vh', overflowY: 'auto' }}>
        ...
        {hasAnyConflicts && (
          ...
          >
            ...
            >
              One or more selected items have scheduling conflicts for the selected dates. You can
              still proceed if needed.
            </p>
            <label
              style={{
                display: 'flex',
        ...
        <div style={{ display: 'flex', gap: spacing[3], justifyContent: 'flex-end' }}>
          ...
          <Button
            ...
            style={
              hasAnyConflicts && acknowledgedConflicts ? { background: colors.warning } : undefined
            }
          >
            {isEdit
              ? 'Save Changes'
