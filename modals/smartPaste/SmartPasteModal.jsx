...
// Main Component
// ============================================================================
export const SmartPasteModal = memo(function SmartPasteModal({
  specs,
  onApply,
  onClose,
  existingItem,
  ...
  const savePasteHistory = useCallback(
    (text, resultSummary) => {
      try {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        /* Community aliases not available */
      }
    })();
    return () => {
      cancelled = true;
  ...
  if (specs) {
    Object.values(specs).forEach((specList) => {
      ...
      specList.forEach((s) => {
        if (s.required) requiredFields.add(s.name);
      });
    });
  }
  ...
  const filteredFields = parseResult
      ...
      try {
        ...
        if (IMAGE_EXTENSIONS.includes(ext)) {
          ...
          try {
            ...
            text = ocrResult.text;
            const confLabel =
              ocrResult.confidence >= 80 ? 'high' : ocrResult.confidence >= 50 ? 'medium' : 'low';
            setImportStatus(
              `success:OCR complete — ${confLabel} confidence (${Math.round(ocrResult.confidence)}%), ${text.split('\n').length} lines`,
            );
          } finally {
            setOcrProgress(null);
          }
  ...
  const handlePaste = useCallback((e) => {
    ...
    if (clipboardData.types.includes('text/html')) {
      ...
      const useHtml =
        htmlTabLines > plainTabLines || (htmlTabLines === plainTabLines && htmlLines > plainLines);
  ...
  const handleDiffExisting = useCallback(
    (existingSpecs) => {
      if (!parseResult) return;
  ...
  return (
    <Modal onClose={onClose} maxWidth={showSourceView ? 1200 : 780}>
      ...
      <div style={{ padding: spacing[4], maxHeight: '80vh', overflowY: 'auto' }}>
        ...
        >
          Paste product specifications from a retailer or manufacturer page, or import a PDF/text
          file. The parser extracts key-value pairs and matches them against your spec fields using
        ...
        >
          ...
          {batchResults && (
            <span
              style={{ fontSize: typography.fontSize.sm, color: colors.primary, fontWeight: 600 }}
            >
          ...
          {parseResult && (
            <span style={{ fontSize: typography.fontSize.sm, color: colors.textMuted }}>
              Extracted <strong style={{ color: colors.textPrimary }}>{totalExtracted}</strong>{' '}
              pairs
              {' → '}
        ...
        {parseResult && (
          <ControlBar
            ...
            setNormalizeMetric={setNormalizeMetric}
            showSourceView={showSourceView}
            setShowSourceView={setShowSourceView}
          />
        )}
        ...
        <BatchSelector
          ...
          onBatchSelectSingle={handleBatchSelectSingle}
        />
