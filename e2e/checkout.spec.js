...
test.describe('Check-Out/Check-In Workflow', () => {
    ...
    test('should open check-out modal for available item', async ({ page, pages }) => {
      ...
      if (await availableItem.isVisible()) {
        ...
        if (await checkOutButton.isVisible()) {
          ...
          const borrowerInput = page.locator('input[id*="borrower"], input[placeholder*="Borrower"], input[placeholder*="Name"]');
    ...
    test('should complete check-out with valid data', async ({ page, pages }) => {
      ...
      if (await availableItem.isVisible()) {
        ...
        if (await checkOutButton.isVisible()) {
          ...
          const borrowerInput = page.locator('input[id*="borrower"], input[placeholder*="Borrower"]').first();
    ...
    test('should show quick due date options', async ({ page, pages }) => {
      ...
      if (await availableItem.isVisible()) {
        ...
        if (await checkOutButton.isVisible()) {
          ...
          const quickDateOptions = page.locator('button:has-text("Tomorrow"), button:has-text("1 week"), button:has-text("3 days")');
    ...
    test('should display checkout information in check-in modal', async ({ page, pages }) => {
      ...
      if (await checkedOutItem.isVisible()) {
        ...
        if (await checkInButton.isVisible()) {
          ...
          const hasBorrowerInfo = await page.locator('text=/Checked out|Borrower|Due/i').isVisible();
    ...
    test('should allow reporting damage during check-in', async ({ page, pages }) => {
      ...
      if (await checkedOutItem.isVisible()) {
        ...
        if (await checkInButton.isVisible()) {
          ...
          const damageCheckbox = page.locator('input[type="checkbox"]').filter({ hasText: /damage/i });
          const damageOption = page.locator('text=/damage|issue|problem/i');
          
          const hasDamageOption = await damageCheckbox.isVisible() || await damageOption.isVisible();
  ...
  test.describe('Dashboard Quick Actions', () => {
    ...
    test('should be able to quick check-out from dashboard', async ({ page }) => {
      ...
      const quickCheckOut = page.locator('button:has-text("Quick Check"), button:has-text("Check Out")');
