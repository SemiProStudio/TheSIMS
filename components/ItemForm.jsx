...
export function useItemForm({
  ...
  const duplicateSerialNumber = useMemo(() => {
    ...
    const duplicate = inventory.find(
      (item) => item.serialNumber?.toLowerCase() === serialLower && item.id !== itemId, // Exclude current item when editing
    );
  ...
  const serialNumberValid =
    !currentCategorySettings.trackSerialNumbers || itemForm.serialNumber?.trim();
