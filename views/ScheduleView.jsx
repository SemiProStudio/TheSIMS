    ...
    <>
        ...
        <Card padding={false}>
          ...
          >
            ...
            <span style={{ fontSize: typography.fontSize.sm, color: colors.textMuted }}>
              {filteredReservations.length} in this period
            </span>
          </div>
          <div style={{ maxHeight: 'calc(100vh - 280px)', overflowY: 'auto' }}>
            {filteredReservations.length === 0 ? (
              ...
              filteredReservations.map((r, idx) => (
                <div
                  key={`${r.project}-${r.start}-${idx}`}
                  onClick={() => onViewReservation(r, r.items[0])}
                  style={{
                    display: 'flex',
                    alignItems: 'center',
                    gap: spacing[4],
                ...
                >
                        ...
                        {itm.image ? (
                          <img
                            ...
                            style={{
                              ...
                              borderRadius: borderRadius.md,
                              objectFit: 'cover',
                              border: `2px solid ${colors.bgMedium}`,
                            }}
                          />
                        ) : (
                          <div
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: typography.fontSize.sm, color: colors.textMuted }}>
                      {r.itemCount > 1
                        ? `${r.items[0]?.name || 'Unknown'} + ${r.itemCount - 1} more`
                ...
                return (
                  ...
                  >
                    ...
                    {rowSegments.map((seg, si) => {
                      ...
                      const label =
                        ev.project ||
                        (ev.itemCount > 1 ? `${ev.itemCount} items` : ev.items[0]?.name) ||
                        'Reservation';
                      const lane = seg.lane ?? 0;
                      ...
                      return (
                        <div
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            onViewReservation(ev, ev.items[0]);
                          }}
                          ...
                          style={{
                            ...
                            background: isHovered
                              ? withOpacity(evColor, 38)
                              : withOpacity(evColor, 20),
      ...
      {scheduleMode === SCHEDULE_MODES.CALENDAR &&
        ...
        (() => {
          ...
          const sorted = [...weekSpanData].sort(
            (a, b) => a.startDay - b.startDay || b.span - a.span,
          );
          const lanes = [];
          sorted.forEach((item) => {
            let assignedLane = -1;
            ...
            <Card padding={false} style={{ overflow: 'hidden' }}>
              ...
              <div style={{ position: 'relative', height: chartHeight, padding: `${BAR_GAP}px 0` }}>
                ...
                {sorted.map((item, idx) => {
                  ...
                  const label =
                    ev.project ||
                    (isMulti ? `${ev.itemCount} items` : ev.items[0]?.name) ||
                    'Reservation';
      ...
      {scheduleMode === SCHEDULE_MODES.CALENDAR &&
        ...
        (() => {
          ...
          return (
            <Card
              padding={false}
              style={{ borderColor: isToday ? colors.primary : undefined, overflow: 'hidden' }}
            >
              <div
                style={{
              ...
              >
                ...
                {events.length > 0 && (
                  <Badge
                    text={`${events.length} reservation${events.length > 1 ? 's' : ''}`}
                    color={colors.primary}
                    size="sm"
                  />
                )}
