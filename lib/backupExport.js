...
export const BACKUP_SECTIONS = [
  ...
  },
  {
    key: 'packages',
    label: 'Kits & Packages',
    tables: ['packages', 'package_items', 'package_notes'],
  },
  { key: 'categories', label: 'Categories & Locations', tables: ['categories', 'locations'] },
  // smart_paste_aliases is the user-taught Smart Paste learning data — losing
  // it on restore silently degrades parsing. (spec_extractions stays out: the
  // table is service-role-only, the client literally cannot read it; same for
  // notification_preferences, whose RLS is own-row-only so an "export" would
  // hold just the exporting admin's row.)
  {
    key: 'specs',
    label: 'Specifications & Smart Paste learning',
    tables: ['specs', 'smart_paste_aliases'],
  },
  {
    key: 'packLists',
    label: 'Pack Lists',
  ...
  { key: 'notifications', label: 'Email Templates', tables: ['email_templates'] },
  { key: 'emailLog', label: 'Email Log', tables: ['notification_log'] },
...
export const DEFAULT_BACKUP_INCLUDE = Object.fromEntries(
  BACKUP_SECTIONS.map((s) => [
    s.key,
    s.key !== 'users' && s.key !== 'auditLog' && s.key !== 'emailLog',
  ]),
);
