...
export async function initErrorTracking() {
  ...
  if (!config.enabled || !config.dsn) {
    console.log('[ErrorTracking] Disabled or no DSN configured');
    isInitialized = true;
    return;
  }
...
export function captureException(error, context = {}) {
  ...
  if (config.environment === 'development') {
    console.error('[ErrorTracking] Exception:', error, enrichedContext);
  }
...
export function setUser(user) {
  ...
  if (config.environment === 'development') {
    console.log('[ErrorTracking] Set user:', userData);
  }
}
