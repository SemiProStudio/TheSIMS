...
export const inventoryService = {
  ...
  async checkOut(
    id,
    { userId, userName, clientId, clientName, project, dueBack, notes, conditionAtCheckout },
  ) {
    const supabase = await db();
    ...
    try {
      historyRow = await checkoutHistoryService.create({
        ...
        action: 'checkout',
        project,
        // The modal collects these and the Item Timeline renders them per
        // checkout entry — they were silently dropped until 2026-08-24
        notes: notes || null,
        condition_at_action: conditionAtCheckout || null,
      });
    } catch (err) {
...
// One home per categories statement — the CRUD methods and syncAll must issue
// identical queries (syncAll once re-implemented them inline, which left the
// CRUD methods unused). Builders return the query unawaited so create/update
// can chain .select().single() while syncAll awaits the bare statement.
const insertCategory = (supabase, row) => supabase.from('categories').insert(row);
const updateCategoryById = (supabase, id, updates) =>
  supabase.from('categories').update(updates).eq('id', id);
const deleteCategoryByName = (supabase, name) =>
...
export const categoriesService = {
  ...
  async create(category) {
    ...
    const { data, error } = await insertCategory(supabase, category).select().single();
  ...
  async delete(name) {
    ...
    const { error } = await deleteCategoryByName(supabase, name);
  ...
  async syncAll(newNames, settings = {}, renames = {}) {
    ...
    for (const name of toDelete) {
      const { error: delErr } = await deleteCategoryByName(supabase, name);
    ...
    for (let i = 0; i < newNames.length; i++) {
      ...
      if (existingByName[name]) {
        // Update existing
        const { error: updateErr } = await updateCategoryById(
          supabase,
        ...
        const { error: insertErr } = await insertCategory(supabase, { name, prefix, ...row });
        if (insertErr) {
...
// ONE DELETE statement for any number of ids, shared by delete() and
// syncAll(). parent_id is ON DELETE CASCADE so order doesn't matter, but the
// result MUST be checked by the caller: the old per-row loop ignored
// refusals, so an RLS or network error left the branch "deleted" in the UI
// and resurrected on the next fetch.
const deleteLocationsByIds = (supabase, ids) => supabase.from('locations').delete().in('id', ids);
...
export const locationsService = {
  ...
  async delete(id) {
    ...
    const { error } = await deleteLocationsByIds(supabase, [id]);
  ...
  async syncAll(tree) {
    ...
    // Delete removed locations in one statement (see deleteLocationsByIds
    // for why it must stay one statement with its error checked)
    const toDelete = [...existingIds].filter((id) => !newIds.has(id));
    if (toDelete.length > 0) {
      const { error: deleteErr } = await deleteLocationsByIds(supabase, toDelete);
...
export const auditLogService = {
  // Filtering (by type / item) happens client-side in AuditLogView over the
  // fetched window — the former server-side type/itemId options were never
  // passed by any caller (2026-08-24 audit §3.7).
...
export const emailService = {
  ...
  async sendCheckoutConfirmation({
    ...
    return this.send({
      ...
      templateData: buildCheckoutConfirmationData({
        borrowerName,
        item,
        checkoutDate,
        dueDate,
  ...
  async sendDamageReport({
    ...
    const templateData = buildDamageReportData({
      item,
      reportedBy,
      borrowerName,
      description,
    ...
    if (recipients.length === 0) {
      return {
        ...
        sent: 0,
        results: [],
        error: 'No admin recipients with an email address',
      };
    }
...
export const notificationLogService = {
  async list({ limit = 200 } = {}) {
    ...
    const { data, error } = await supabase
      ...
      .select(
        'id, email, notification_type, subject, status, error_message, item_id, external_id, sent_at, created_at',
      )
      .order('created_at', { ascending: false })
      .limit(limit);
    if (error) throw error;
