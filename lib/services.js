...
export const packListsService = {
  ...
  async create(packList) {
    ...
    if (items?.length > 0) {
      ...
      if (itemsError) warn('Failed to insert pack list items:', itemsError.message);    }
    ...
    if (packages?.length > 0) {
      ...
      if (pkgError) warn('Failed to insert pack list packages:', pkgError.message);    }
  ...
  async update(id, updates) {
    ...
    if (items !== undefined) {
      ...
      if (delError) warn('Failed to delete pack list items:', delError.message);      if (items.length > 0) {
        ...
        if (insError) warn('Failed to insert pack list items:', insError.message);      }
    }

    // Update packages if provided
    if (packages !== undefined) {
      const { error: delError } = await supabase.from('pack_list_packages').delete().eq('pack_list_id', id);
      if (delError) warn('Failed to delete pack list packages:', delError.message);      if (packages.length > 0) {
        ...
        if (insError) warn('Failed to insert pack list packages:', insError.message);      }
    }
