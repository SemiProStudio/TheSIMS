...
// INVENTORY SERVICE
// =============================================================================
// Helper to transform database field names to frontend field names
function transformInventoryItem(item) {
  ...
  return {
    ...
    // Checkout state
    checkedOutTo: item.checked_out_to_name || item.checkedOutTo,
    checkedOutToUserId: item.checked_out_to_user_id || item.checkedOutToUserId,
    checkedOutDate: item.checked_out_date || item.checkedOutDate,
    dueBack: item.due_back || item.dueBack,
    checkoutProject: item.checkout_project || item.checkoutProject,
    checkoutClientId: item.checkout_client_id || item.checkoutClientId,
    // Kit fields
    isKit: item.is_kit || item.isKit || false,
    kitType: item.kit_type || item.kitType,
    kitItems: item.kit_contents || item.kitItems || [],
    // Stats
    viewCount: item.view_count || item.viewCount || 0,
    checkoutCount: item.checkout_count || item.checkoutCount || 0,
    // Location
...
// Helper to transform frontend field names to database field names
function transformItemForDb(item) {
  if (!item) return null;
  
  // Build the database-formatted object
  const dbItem = {
    id: item.id,
    name: item.name,
    brand: item.brand,
  ...
  if (item.purchaseDate || item.purchase_date) {
    dbItem.purchase_date = item.purchaseDate || item.purchase_date;
  }
  if (item.purchasePrice || item.purchase_price) {
    dbItem.purchase_price = Number(item.purchasePrice || item.purchase_price) || 0;
  }
  if (item.currentValue || item.current_value) {
    dbItem.current_value = Number(item.currentValue || item.current_value) || 0;
  }
  if (item.serialNumber || item.serial_number) {
    dbItem.serial_number = item.serialNumber || item.serial_number;
  }
  if (item.reorderPoint || item.reorder_point) {
    dbItem.reorder_point = Number(item.reorderPoint || item.reorder_point) || 0;
  }
  if (item.location || item.location_display) {
    dbItem.location_display = item.location || item.location_display;
  }
  if (item.locationId || item.location_id) {
    dbItem.location_id = item.locationId || item.location_id;
  }
  if (item.isKit || item.is_kit) {
    dbItem.is_kit = item.isKit || item.is_kit;
  }
  if (item.kitType || item.kit_type) {
    dbItem.kit_type = item.kitType || item.kit_type;
  }
  if (item.kitItems || item.kit_contents) {
    dbItem.kit_contents = item.kitItems || item.kit_contents;
  }
...
export const inventoryService = {
  ...
  async create(item) {
    ...
    const dbItem = transformItemForDb(item);
  ...
  async checkOut(id, { userId, userName, clientId, clientName, project, dueBack }) {
    ...
    supabase.rpc('increment_checkout_count', { item_id: id })
      .then(({ error: rpcErr }) => {
        if (rpcErr) console.warn('increment_checkout_count RPC not available:', rpcErr.message);
      });
    ...
    checkoutHistoryService.create({
      ...
      action: 'checkout',
      project
    }).catch(err => console.warn('Checkout history not saved:', err.message));
  ...
  async checkIn(id, { userId, userName, notes, condition }) {
    ...
    }).catch(err => console.warn('Checkin history not saved:', err.message));
...
function transformReminder(reminder) {
  ...
  return {
    ...reminder,
    itemId: reminder.item_id || reminder.itemId,
    dueDate: reminder.due_date || reminder.dueDate,
    completedAt: reminder.completed_at || reminder.completedAt,
    completedDate: reminder.completed_at || reminder.completedDate,
    createdById: reminder.created_by_id || reminder.createdById,
    createdByName: reminder.created_by_name || reminder.createdByName,
    ...
    item: reminder.item ? transformInventoryItem(reminder.item) : reminder.item,
  };
}
...
function transformReservation(res) {
  ...
  return {
    ...res,
    // Notes - ensure it's always an array
    notes,
    // Date fields
    start: res.start_date || res.start,
    end: res.end_date || res.end,
    startDate: res.start_date || res.start,
    endDate: res.end_date || res.end,
    // Contact fields
    contactName: res.contact_name || res.contactName,
    contactPhone: res.contact_phone || res.contactPhone,
    contactEmail: res.contact_email || res.contactEmail,
    // IDs
    itemId: res.item_id || res.itemId,
    clientId: res.client_id || res.clientId,
    createdById: res.created_by_id || res.createdById,
    createdByName: res.created_by_name || res.createdByName,
    // Other
    projectType: res.project_type || res.projectType,
    createdAt: res.created_at || res.createdAt,
    updatedAt: res.updated_at || res.updatedAt,
    // Transform nested item if present
    item: res.item ? transformInventoryItem(res.item) : res.item,
  };
}
...
function transformMaintenanceRecord(record) {
  ...
  return {
    ...
    completedDate: record.completed_date || record.completedDate,
    warrantyWork: record.warranty_work || record.warrantyWork || false,
    vendorContact: record.vendor_contact || record.vendorContact,
    createdById: record.created_by_id || record.createdById,
    createdByName: record.created_by_name || record.createdByName,
    createdAt: record.created_at || record.createdAt,
    updatedAt: record.updated_at || record.updatedAt,
    // Transform nested item if present
    item: record.item ? transformInventoryItem(record.item) : record.item,
  };
}
...
function transformCheckoutHistory(record) {
  ...
  return {
    ...
    conditionAtAction: record.condition_at_action || record.conditionAtAction,
