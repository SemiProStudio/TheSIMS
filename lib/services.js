...
export const inventoryService = {
  ...
  // Check in item
  async checkIn(id, { userId, userName, notes, condition, damageReported }) {
    const supabase = await db();
    ...
    const updates = {
      status: damageReported ? 'needs-attention' : 'available',
...
export const packListsService = {
  ...
  async create(packList) {
    ...
    // Add items
    if (items?.length > 0) {
      const { error: itemsError } = await supabase.from('pack_list_items').insert(
    ...
    if (packages?.length > 0) {
      const { error: pkgError } = await supabase.from('pack_list_packages').insert(
  ...
  async update(id, updates) {
    ...
    if (items !== undefined) {
      const { error: delError } = await supabase
        ...
        .delete()
        .eq('pack_list_id', id);
      if (delError) warn('Failed to delete pack list items:', delError.message);
      if (items.length > 0) {
    ...
    if (packages !== undefined) {
      const { error: delError } = await supabase
        ...
        .delete()
        .eq('pack_list_id', id);
      if (delError) warn('Failed to delete pack list packages:', delError.message);
      if (packages.length > 0) {
...
export const specsService = {
  ...
  async upsert(categoryName, specs) {
    ...
    await supabase.from('specs').delete().eq('category_name', categoryName);
