...
export const reservationsService = {
  ...
  async create(reservation) {
    const { validateReservation } = await import('./validators.js');
...
export const packagesService = {
  ...
  async create(pkg) {
    ...
    // Generate short human-readable ID (PKG-001, PKG-002, etc.)
    if (!packageData.id) {
      ...
      if (existing?.length > 0) {
        const match = existing[0].id.match(/^PKG-(\d+)$/);
        if (match) nextNum = parseInt(match[1], 10) + 1;
      }
      packageData.id = `PKG-${String(nextNum).padStart(3, '0')}`;
    }
...
export const packListsService = {
  ...
  async create(packList) {
    ...
    const { items, packages, packedItems: _packedItems, ...listData } = packList;
    ...
    if (items?.length > 0) {
      ...
      if (itemsError) console.warn('Failed to insert pack list items:', itemsError.message);    }
    ...
    if (packages?.length > 0) {
      ...
      if (pkgError) console.warn('Failed to insert pack list packages:', pkgError.message);    }
  ...
  async update(id, updates) {
    ...
    // Update items if provided
    if (items !== undefined) {
      const { error: delError } = await supabase.from('pack_list_items').delete().eq('pack_list_id', id);
      if (delError) console.warn('Failed to delete pack list items:', delError.message);      if (items.length > 0) {
        const { error: insError } = await supabase.from('pack_list_items').insert(
        ...
        if (insError) console.warn('Failed to insert pack list items:', insError.message);      }
    }

    // Update packages if provided
    if (packages !== undefined) {
      const { error: delError } = await supabase.from('pack_list_packages').delete().eq('pack_list_id', id);
      if (delError) console.warn('Failed to delete pack list packages:', delError.message);      if (packages.length > 0) {
        const { error: insError } = await supabase.from('pack_list_packages').insert(
        ...
        if (insError) console.warn('Failed to insert pack list packages:', insError.message);      }
    }
...
export const categoriesService = {
   ...
   */
  async syncAll(newNames, settings = {}) {
    const supabase = await db();
    ...
    for (const name of toDelete) {
      ...
      await supabase.from('specs').delete().eq('category_name', name);
    }
    ...
    for (let i = 0; i < newNames.length; i++) {
      ...
      if (existingByName[name]) {
        // Update existing
        const { error: updateErr } = await supabase.from('categories').update(row).eq('id', existingByName[name].id);
        if (updateErr) warn(`Failed to update category "${name}":`, updateErr);
        ...
        const { error: insertErr } = await supabase.from('categories').insert({ name, prefix, ...row });
        if (insertErr) warn(`Failed to create category "${name}":`, insertErr);
      }
    }
  }
};
...
export const auditLogService = {
  ...
  async create(entry) {
    ...
    const { type: _type, description: _desc, user: _user, userName: _uName, userId: _uId,
            itemId: _iId, clientId: _cId, packageId: _pkgId, packListId: _plId,
            reservationId: _rId, timestamp: _ts, ...extra } = entry;
