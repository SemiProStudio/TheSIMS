  ...
  async syncAll(newNames, settings = {}) {
    ...
    for (let i = 0; i < newNames.length; i++) {
      ...
      if (existingByName[name]) {
        // Update existing
        await supabase.from('categories').update(row).eq('id', existingByName[name].id);
      } else {
        // Create new — generate a prefix from the name
        const prefix = name.substring(0, 2).toUpperCase();
        await supabase.from('categories').insert({ name, prefix, ...row });
      }
    }
  }
};
