...
const THUMB_CAP_BYTES = 256 * 1024;
export const FULL_QUALITY_LADDER = [0.85, 0.78, 0.7, 0.6];
const THUMB_QUALITY_LADDER = [0.82, 0.75, 0.65];
...
// How long to wait for canvas.toBlob before falling back to the synchronous
// encoder. Safari has been seen never calling back when its compositor is
// wedged (the upload sat on "Uploading…" forever, no request ever sent).
export const TO_BLOB_TIMEOUT_MS = 8000;
...
export function canvasToBlob(canvas, type, quality, { timeoutMs = TO_BLOB_TIMEOUT_MS } = {}) {
  return new Promise((resolve, reject) => {
    ...
    // Watchdog: the synchronous encoder doesn't depend on an async callback
    // the browser may drop
    const timer = setTimeout(() => {
      try {
        finish(resolve, dataUrlToBlob(canvas.toDataURL(type, quality)));
      } catch (err) {
        finish(reject, err instanceof Error ? err : new Error('Image encoding failed'));
      }
    }, timeoutMs);
    try {
      canvas.toBlob(
        (blob) =>
          blob ? finish(resolve, blob) : finish(reject, new Error('Image encoding failed')),
        type,
        quality,
      );
    } catch (err) {
...
function drawScaled(drawable, sx, sy, sw, sh, dw, dh) {
  ...
  canvas.width = dw;
  canvas.height = dh;
  // willReadFrequently keeps the canvas CPU-backed: encoding (toBlob /
  // toDataURL) then needs no GPU readback, which is the step Safari has
  // been seen to stall on. A one-off drawImage at these sizes is cheap on
  // the CPU.
 ...
 */
function renderRendition(working, { maxEdge, square = false, crop = null }) {
