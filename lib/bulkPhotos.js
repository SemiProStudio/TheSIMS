...
export function stripSequenceSuffix(stem) {
  // The digits need an explicit delimiter — separator chars or parentheses.
  // Bare trailing digits are part of the name ("cam1002" must not become
  // "cam1").
  return stem.replace(/(?:[\s_-]+\(?\d{1,3}\)?|\s*\(\d{1,3}\))$/, '').trim();
}
