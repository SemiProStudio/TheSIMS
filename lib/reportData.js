...
import { INVENTORY_COLUMNS, inventoryLabel } from './inventoryCsv.js';
...
/**
 * CLIENT-LINKED bookings per month over the trailing window, oldest first.
 * Filtered to match computeClientReportStats — the trend used to count every
 * group while the stat cards counted only client-linked ones, so five
 * clientless bookings showed as a spike the table couldn't explain.
 */
export const bookingsSeries = (inventory, { months = 12, now = new Date() } = {}) => {
  const keys = lastMonthKeys(months, now);
  const counts = Object.fromEntries(keys.map((k) => [k, 0]));
  groupReservationsForSchedule(inventory).forEach((group) => {
    if (!group.start || !group.clientId) return;
...
export const acquisitionSeries = (inventory, { months = 24, now = new Date() } = {}) => {
  ...
  const series = keys.map((key) => {
    ...
    // Spans over a year repeat bare month names ("Sep", "Sep") — carry the
    // year so the axis is unambiguous (and React keys stay unique)
    const label = months > 12 ? `${monthLabelOf(key)} '${key.slice(2, 4)}` : monthLabelOf(key);
  ...
  VALUE_BUCKETS.map(({ label, max }, idx) => {
    ...
    return {
      ...
      value: inventory.filter((i) => {
        // Clamp corrupt negative values into the lowest band — dropping them
        // made the histogram total disagree with the item count
        const v = Math.max(0, i.currentValue || 0);
...
export const bucketEvents = (events, { days, now = new Date() } = {}) => {
  ...
  if (unit === 'day') {
    ...
    // Month keys from the WINDOW START's month through now — a fixed
    // last-N-months list dropped events in the partial oldest month (they
    // passed the date filter but had no bucket), so the trend disagreed
    // with the day-of-week chart sitting next to it by up to ~4 weeks.
...
// Columns from the shared inventory definition — this exporter used to carry
// its own list with a third id-header spelling ("Item ID") and no
// reorderPoint column
export const csvForInventory = (items, now = new Date()) => ({
  headers: INVENTORY_COLUMNS.map((c) => c.label),
  rows: items.map((item) => INVENTORY_COLUMNS.map((c) => c.value(item))),
  filename: `inventory-summary-${csvDate(now)}.csv`,
});

// Headers for fields that ARE inventory columns come from the shared
// definition (one id spelling everywhere); report-specific columns stay
// literal. Old "Item ID" exports still import via the legacy alias.
export const csvForActivity = (inventory, now = new Date()) => ({
  headers: [
    inventoryLabel('id'),
    inventoryLabel('name'),
    inventoryLabel('brand'),
    inventoryLabel('category'),
...
export const csvForAlerts = (allAlerts, now = new Date()) => ({
  headers: [
    ...
    inventoryLabel('category'),
    inventoryLabel('status'),
    inventoryLabel('condition'),
    inventoryLabel('location'),
    inventoryLabel('currentValue'),
    'Alert Reasons',
    'Due Back',
    'Checked Out To',
  ],
...
export const csvForMaintenance = (records, now = new Date()) => ({
  // 'Status' here is the RECORD's status (scheduled/completed), not the
  // item's — it stays literal on purpose
  headers: [
    'Item',
    inventoryLabel('id'),
    'Type',
...
export const csvForInsurance = (items, now = new Date()) => ({
  headers: [
    ...
    inventoryLabel('brand'),
    inventoryLabel('category'),
    inventoryLabel('serialNumber'),
    inventoryLabel('purchaseDate'),
    inventoryLabel('purchasePrice'),
    inventoryLabel('currentValue'),
    inventoryLabel('condition'),
    inventoryLabel('location'),
    inventoryLabel('status'),
