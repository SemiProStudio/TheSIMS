...
export const INVENTORY_COLUMNS = [
  ...
  // Per-item opt-in; exported as yes/no so a re-import keeps it
  {
    id: 'lowStockAlert',
    label: 'Low Stock Alert',
    value: (i) => (i.lowStockAlert ? 'yes' : 'no'),
    dbValue: (r) => (r.low_stock_alert ? 'yes' : 'no'),
  },
];

export const inventoryColumnById = Object.fromEntries(INVENTORY_COLUMNS.map((c) => [c.id, c]));

// Canonical display header for one inventory field — the report CSV builders
// reuse these instead of hand-rolling spellings, so the id column has ONE
// header ("ID") across every exporter. "Item ID" survives only as an import
// alias for files exported before the consolidation.
export const inventoryLabel = (id) => inventoryColumnById[id].label;
