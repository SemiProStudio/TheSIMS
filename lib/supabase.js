...
export const auth = {
  ...
  // Create a user on behalf of an admin WITHOUT touching the admin's own
  // session. signUp on the shared client returns a session for the NEW user
  // when email confirmation is off — which used to silently replace the
  // admin's login with the account they just created. A throwaway,
  // non-persisting client keeps the new session fully isolated.
