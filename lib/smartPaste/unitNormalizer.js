...
export function normalizeUnits(value, preferMetric = true) {
  ...
  const dimInches = value.match(
    /([\d.]+)\s*[x×]\s*([\d.]+)(?:\s*[x×]\s*([\d.]+))?\s*(?:in(?:ch(?:es)?)?|")/i,
  );
  if (dimInches && preferMetric) {
    const dims = [dimInches[1], dimInches[2], dimInches[3]]
      .filter(Boolean)
      .map((d) => Math.round(parseFloat(d) * 25.4));
  ...
  if (dimMm && !preferMetric) {
    const dims = [dimMm[1], dimMm[2], dimMm[3]]
      .filter(Boolean)
      .map((d) => (parseFloat(d) / 25.4).toFixed(2));
  ...
  if (preferMetric) {
    for (const key of ['in_to_mm', 'lb_to_g', 'oz_to_g']) {
      ...
      if (match) {
        ...
        const converted = conv.fn
          ? conv.fn(num)
          : (num * conv.factor).toFixed(conv.toUnit === 'mm' ? 1 : 0);
...
export function coerceFieldValue(specName, value) {
  ...
  // One direction only: a spec whose name merely appears INSIDE a bool-field
  // name is not boolean itself ('Power' ⊂ 'phantom power' turned a Power spec
  // of "None" into "No")
  if (boolFields.some((f) => nameLower.includes(f))) {
