 ...
 * @param {string} [authToken] - The signed-in user's session access token.
 *   The function's isTrustedCaller gate rejects the bare anon key with 401,
 *   so callers must pass a real user token.
 * @returns {Promise<{text: string, html: string, sourceUrl: string}>}
 */
export async function fetchProductPage(url, proxyUrl, authToken) {
  if (!proxyUrl) {
    throw new Error(
  ...
  if (!response.ok) {
    ...
    // 422 = remote site blocked the request (bot protection, etc.)
