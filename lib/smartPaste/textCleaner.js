...
export function cleanInputText(text) {
  ...
  cleaned = cleaned.replace(
    /<(?:picture|video|audio|iframe|canvas|object|embed)[^>]*(?:\/>|>[\s\S]*?<\/(?:picture|video|audio|iframe|canvas|object|embed)>)/gi,
    '',
  );
  cleaned = cleaned.replace(
    /<(?:script|style|noscript)[\s\S]*?<\/(?:script|style|noscript)>/gi,
    '',
  );
  cleaned = cleaned.replace(/<!--[\s\S]*?-->/g, '');
  // Remove nav, footer, form, button blocks
  cleaned = cleaned.replace(
    /<(?:button|nav|footer|form)[^>]*>[\s\S]*?<\/(?:button|nav|footer|form)>/gi,
    '',
  );
  ...
  cleaned = cleaned.replace(
    /<\/(?:p|div|tr|li|h[1-6]|dt|dd|section|article|header|blockquote)>/gi,
    '\n',
  );
  ...
  cleaned = cleaned.replace(/&#[xX]([0-9a-fA-F]+);/g, (_, code) =>
