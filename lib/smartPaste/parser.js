...
function extractPrice(rawPairs, fullText) {
  ...
  for (const pair of rawPairs) {
    ...
    if (priceLabelPattern.test(keyNorm)) {
      // Exact match, not includes — 'retail price' contains 'price' and was
      // outranking its own dedicated entry (rank 3 vs 7), inverting the order
      const rank = priceKeyPriority.findIndex((p) => keyNorm === p);
...
/**
 * One-to-one assignment: each source pair commits to at most one spec field —
 * its best-scoring candidate (ties prefer the detected category's own specs).
 * Without this, a single line like "Focus Type: Autofocus" fanned out into
 * several fields (Autofocus, AF System, AF Points, Focus Gear Pitch) and
 * near-duplicate specs all received the same value. Multiple pairs may still
 * land on the same spec — resolveFields keeps its merge/conflict handling
 * for that legitimate case.
 */
function assignPairsToFields(candidateMap, specCategories, detectedCategory) {
  // Group every candidate by the source pair it came from
  const byPair = new Map();
  for (const [specName, candidates] of candidateMap) {
    for (const c of candidates) {
      const list = byPair.get(c.pairIndex) || [];
      list.push({ ...c, specName });
      byPair.set(c.pairIndex, list);
    }
  }

  const assigned = new Map();
  for (const specName of candidateMap.keys()) assigned.set(specName, []);

  // An in-category spec within this margin of the overall best wins — the
  // value should land where the item can actually hold it (e.g. a lens's
  // "Dimensions (ø x L)" belongs in Lenses' "Dimensions (DxL)", not the
  // shared cross-category "Dimensions", even if the latter scores higher).
  const IN_CATEGORY_MARGIN = 15;

  for (const list of byPair.values()) {
    list.sort((a, b) => {
      if (b.confidence !== a.confidence) return b.confidence - a.confidence;
      // Tie-break: prefer a spec belonging to the detected category
      const aCat = detectedCategory && specCategories.get(a.specName) === detectedCategory ? 1 : 0;
      const bCat = detectedCategory && specCategories.get(b.specName) === detectedCategory ? 1 : 0;
      return bCat - aCat;
    });
    let best = list[0];
    if (detectedCategory && specCategories.get(best.specName) !== detectedCategory) {
      const inCategory = list.find((c) => specCategories.get(c.specName) === detectedCategory);
      if (inCategory && inCategory.confidence >= best.confidence - IN_CATEGORY_MARGIN) {
        best = inCategory;
      }
    }
    assigned.get(best.specName).push(best);
  }
...
export function parseProductText(text, specsConfig, { communityAliases } = {}) {
  ...
  // 7.5. One-to-one: each source pair commits to its single best field
  const assignedMap = assignPairsToFields(candidateMap, specCategories, result.category);

  // 8. Resolve best match per field
  result.fields = resolveFields(assignedMap);
