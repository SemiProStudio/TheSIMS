...
const SHARED_FIELDS = new Set(['Weight', 'Dimensions', 'Battery Type', 'Battery Life', 'Mount Type', 'Power Input', 'Material']);
...
function injectCommunityAliases(aliasMap, communityAliases) {
  ...
  for (const [sourceKey, { specName, usageCount }] of communityAliases) {
    ...
    if (!aliasMap.has(norm)) {
      const priority = Math.min(CONFIDENCE.COMMUNITY_MAX, CONFIDENCE.COMMUNITY_BASE + Math.floor((usageCount - 3) * 1.5));
  ...
  rawPairs.forEach((pair, idx) => {
    ...
    for (const specName of allSpecNames) {
      ...
      if (score >= CONFIDENCE.FUZZY_MINIMUM) {
        ...
        if (detectedCategory && specCat && specCat !== detectedCategory && !SHARED_FIELDS.has(specName)) {
    ...
    for (const [alias, entry] of aliasMap.entries()) {
      ...
      if (score >= CONFIDENCE.FUZZY_ALIAS_MINIMUM) {
        ...
        if (detectedCategory && specCat && specCat !== detectedCategory && !SHARED_FIELDS.has(entry.specName)) {
          adj = Math.max(0, adj - 25);
        }
        if (adj >= CONFIDENCE.FUZZY_MINIMUM) fuzzyMatches.push({ specName: entry.specName, score: adj });
      }
    }
...
function resolveFields(candidateMap) {
  ...
  for (const [specName, candidates] of candidateMap) {
    ...
    if (!merged && deduped.length > 1) {
      ...
      if (confDiff <= CONFIDENCE.CONFLICT_DIFF_THRESHOLD && topTwo[0].confidence >= CONFIDENCE.FUZZY_MINIMUM && topTwo[1].confidence >= CONFIDENCE.FUZZY_MINIMUM) {
        hasConflict = true;
      }
    }
...
export function parseProductText(text, specsConfig, { communityAliases } = {}) {
  ...
  const { candidateMap, matchedPairIndices } = matchFields(rawPairs, aliasMap, allSpecNames, specCategories, result.category);
