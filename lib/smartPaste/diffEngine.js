...
export function diffSpecs(existingSpecs, newFields) {
  ...
  for (const specName of allKeys) {
    // Stored spec values aren't guaranteed to be strings (numeric specs) —
    // coerce before the toLowerCase/trim comparison below
    const oldVal = String(existingSpecs?.[specName] || '');
    const newField = newFields?.get(specName);
    const newVal = String(newField?.value || '');
