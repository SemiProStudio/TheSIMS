...
export function similarityScore(source, target) {
  ...
  if (wordsA.length > 0 && wordsB.length > 0) {
    ...
    for (const wa of wordsA) {
      ...
      if (wa.length >= 5) {
        const fuzzyIdx = wordsB.findIndex(
          (wb, i) => !usedB.has(i) && wb.length >= 5 && levenshtein(wa, wb) <= 1,
        );
        if (fuzzyIdx !== -1) {
          fuzzyShared++;
          usedB.add(fuzzyIdx);
    ...
    if (exactShared === 1 && wordsA.find((w) => wordsB.includes(w) && w.length >= 7))
      return CONFIDENCE.SINGLE_LONG_WORD;
    if (exactShared === 1 && wordsA.find((w) => wordsB.includes(w) && w.length >= 5))
      return CONFIDENCE.SINGLE_MEDIUM_WORD;
  }
