...
export function applySmartPastePayload(prev, parsed) {
  const specs = { ...prev.specs, ...parsed.specs };
  // The extracted Model # lands as a spec — it was carried in the payload,
  // shown in the review panel, and then dropped here (the same gap this
  // function once had for serialNumber). A parsed 'Model' spec wins over
  // the standalone extraction, matching the parsed-over-previous merge rule.
  if (parsed.modelNumber && !parsed.specs?.Model) {
    specs.Model = parsed.modelNumber;
  }
  return {
    ...prev,
