...
const HEADER_ALIASES = {
  ...
  'serial #': 'serialNumber',
  serial: 'serialNumber',
  // Legacy alias: report exporters labeled the id column "Item ID" before
  // the header consolidation (they emit the canonical "ID" now) — old files
  // must keep importing
  'item id': 'id',
...
export function buildImportItems(
  ...
  if (unknown.length > 0) {
    warnings.push(
      `Ignored unrecognized column${unknown.length === 1 ? '' : 's'}: ${unknown.join(', ')}`,
    );
  }
