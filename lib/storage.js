...
export const storageService = {
  ...
  async uploadImage(file, itemId) {
    ...
    try {
      ...
      const [fullResult, thumbResult] = await Promise.all([
        supabase.storage.from(BUCKET_NAME).upload(`${basePath}.jpg`, fullBlob, {
          contentType: 'image/jpeg',
          cacheControl: '31536000', // 1 year cache
          upsert: false,
        }),
        supabase.storage.from(BUCKET_NAME).upload(`${basePath}_thumb.jpg`, thumbBlob, {
          contentType: 'image/jpeg',
  ...
  async uploadFromDataUrl(dataUrl, itemId) {
    ...
    const extension = dataUrl.includes('image/png')
      ? 'png'
