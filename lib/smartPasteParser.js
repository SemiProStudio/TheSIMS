 ...
 */
export function parseBatchProducts(text, specs) {
  const segments = detectProductBoundaries(text);
  if (segments.length === 0) {
    // Single product — return as array of one
    return [{ segment: { name: 'Single Product', text }, result: parseProductText(text, specs) }];
  }
  return segments.map(segment => ({
    segment,
    result: parseProductText(segment.text, specs),
  }));
}
...
export async function recordAlias(supabase, sourceKey, specName, category) {
  ...
  try {
    ...
    if (error) console.warn('Failed to record alias:', error.message);
  } catch (e) {
    console.warn('Community alias recording not available:', e.message);
  }
}
...
export async function fetchCommunityAliases(supabase, minUsage = 3) {
  ...
  try {
    ...
    console.warn('Community aliases not available:', e.message);
    return new Map();
  }
}
