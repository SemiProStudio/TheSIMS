...
import { parseCurrency } from './validators.js';
 ...
 * Thin wrapper over parseCurrency's strict mode — the CSV import and the
 * form paths (lenient mode: junk → 0) share one parser.
 */
export function parseMoney(str) {
  const value = parseCurrency(str, { strict: true });
  return value === null ? { value: 0, ok: false } : { value, ok: true };
}
