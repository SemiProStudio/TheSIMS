...
/**
 * Parse a human money string ("$3,498.50" → 3498.5) — the ONE parser behind
 * both consumers, split by junk handling:
 * - lenient (default; form/validator paths): parseFloat semantics — junk
 *   coerces to 0, a numeric prefix parses ("12abc" → 12), numbers pass through
 * - strict ({ strict: true }; CSV import via parseMoney): the entire cleaned
 *   string must be numeric; junk returns null so the caller can report the
 *   bad cell instead of silently storing 0. Empty/absent input is still 0.
 */
export function parseCurrency(value, { strict = false } = {}) {
  if (strict) {
 ...
 * @param {boolean} options.skipCategoryCheck - Trust the caller's category (it
 *   validated against the LIVE category list; this module only knows the
 *   hardcoded defaults, so re-checking here would reject every admin-added
 *   or renamed category)
...
export function validateItem(data, options = {}) {
  ...
  } else if (!skipCategoryCheck && !isValidCategory(data.category, customCategories)) {
  ...
  // Current value validation — real items carry currentValue, not value;
  // checking only data.value left every actual item value unvalidated
  if (data.currentValue !== undefined && data.currentValue !== '' && data.currentValue !== null) {
    const numCurrent = parseCurrency(data.currentValue);
    if (numCurrent < 0) {
      errors.currentValue = 'Value cannot be negative';
    } else if (numCurrent > 10000000) {
      errors.currentValue = 'Value exceeds maximum allowed';
    }
  }
  ...
  const sanitizedData = isValid
    ? {
        ...
        value:
          data.value !== undefined && data.value !== '' ? parseCurrency(data.value) : undefined,
        // currentValue is the field real items carry (see the validation
        // above) — it must go through the same currency sanitization or a
        // "$3,498.50" string reaches storage as-is
        currentValue:
          data.currentValue !== undefined && data.currentValue !== '' && data.currentValue !== null
            ? parseCurrency(data.currentValue)
            : undefined,
        purchasePrice:
          data.purchasePrice !== undefined && data.purchasePrice !== ''
            ? parseCurrency(data.purchasePrice)
            : undefined,
      }
    : null;
...
export function validateReservation(data, options = {}) {
  ...
  // Date range validation — the rule allows start === end (same-day
  // reservations are legitimate), so the copy says "before", not "after"
...
export function validateItems(items, options = {}) {
  ...
  for (let i = 0; i < items.length; i++) {
    const item = items[i];
    // Only the OTHER items' codes — including the item's own code flagged
    // every coded item as a duplicate of itself
