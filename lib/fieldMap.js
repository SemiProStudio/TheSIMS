...
export const INVENTORY_FIELD_MAP = {
  ...
  // Accessory pairing (JSONB array of inventory ids)
...
export const REMINDER_FIELD_MAP = {
  itemId: 'item_id',
  dueDate: 'due_date',
  // completed_at maps ONLY to completedDate. A duplicate completedAt entry
  // used to sit above it — the reverse map picked completedDate (later key
  // wins), so rows never carried completedAt and readers of it got undefined.
...
function reverseMap(fieldMap) {
  ...
  const reversed = Object.fromEntries(Object.entries(fieldMap).map(([fe, db]) => [db, fe]));
...
// Detect database column names that are date/timestamp typed.
// Empty strings must become null for these columns or Postgres rejects them.
const DATE_COLUMN_RE = /(?:_(?:date|at)$|^due_back$)/;
  ...
  for (const [feKey, dbKey] of Object.entries(fieldMap)) {
    ...
    if (value !== undefined) {
      ...
      if (feKey in numericFields) {
        ...
        result[dbKey] = value === '' && DATE_COLUMN_RE.test(dbKey) ? null : value;
      }
    }
  }
